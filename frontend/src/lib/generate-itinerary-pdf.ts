...
function buildPdfHtml(data: ItineraryPdfData, fullRouteUrl: string | null): string {
  ...
  const daysHtml = data.itinerary.map(day => {
    ...
    const activitiesHtml = sorted.map((act, idx) => {
      const imgs = getActivityImages(act).slice(0, 2);
      const mapsUrl = act.maps_url?.trim() || fallbackMapsSearchUrl(act, data.destination);
      // data-pdf-link is read by the post-processor to inject clickable jsPDF link annotations
      return `
        <div class="activity">
...
interface LinkAnnotation {
  url: string;
  /** Position in CSS pixels relative to the rendered root */
  x: number;
  y: number;
  w: number;
  h: number;
...
export async function generateItineraryPdf(data: ItineraryPdfData): Promise<void> {
  ...
  try {
    // Collect link annotations BEFORE rasterizing
    ...
    // pixel (CSS) → mm conversion factor
    ...
    const addLinksForPage = (pageOffsetMm: number) => {
      annotations.forEach((a) => {
        ...
        // Only add link if it falls within current page bounds
