...
export default function ClienteRoteiro() {
  ...
  // Fetch proposal with itinerary
  const { data: proposal, isLoading } = useQuery({
    queryKey: ['client-proposal-itinerary', id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('proposals')
        .select('*, travel_requests!inner(destination, travel_dates, travelers_count, preferences, client_id)')
        .eq('request_id', id!)
        .eq('is_approved', true)
        .maybeSingle();
  ...
  const saveItinerary = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['client-proposal-itinerary', id] });
    },
  });
  ...
  const generateFullItinerary = async () => {
    ...
    try {
      ...
      if (newDays) {
        const merged = newDays.map(d => ({
          ...d,
          activities: d.activities.map(a => ({ ...a, is_suggestion: true })),
        }));
  ...
  const suggestForDay = async (dayNum: number) => {
    ...
    try {
      ...
      if (suggestedDay) {
        ...
        if (existingIdx >= 0) {
          updated[existingIdx].activities.push(...newActivities);
        } else {
          updated.push({ day: dayNum, activities: newActivities });
        }
  ...
  const addEmptyDay = async () => {
    ...
    const updated = [...itinerary, { day: nextDay, activities: [] }];
    await saveItinerary.mutateAsync(updated);
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <Button className="mt-4" asChild>
          <Link to={`/minha-conta/viagem/${id}`}>Voltar para Viagem</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild>
          <Link to={`/minha-conta/viagem/${id}`}><ArrowLeft className="h-4 w-4" /></Link>
        </Button>
          ...
          {itinerary.map((day, dayIdx) => {
            ...
            return (
              <div key={day.day} className="relative pl-16">
                {/* Day node */}
                <div className="absolute left-3 top-4 z-10 flex h-7 w-7 items-center justify-center rounded-full bg-primary text-primary-foreground text-xs font-bold">
                  {day.day}
                </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-muted-foreground">
                          R$ {dayCost.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </span>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => suggestForDay(day.day)}
                  ...
                  <CardContent className="space-y-2">
                    {sorted.length === 0 && (
                      <p className="text-sm text-muted-foreground py-4 text-center">
                        Nenhuma atividade. Clique em "Sugerir mais" para pedir à IA.
                      </p>
                    )}
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-lg border p-3 space-y-1 transition-colors ${
                            activity.is_suggestion ? 'border-dashed border-primary/50 bg-primary/5' : ''
                          }`}
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <Badge className={`text-xs ${categoryColors[activity.category] || 'bg-muted text-muted-foreground'}`}>
                                  {activity.category}
                                </Badge>
                                {activity.is_suggestion && (
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              <span className="text-sm font-medium">
                                R$ {(activity.estimated_cost || 0).toLocaleString('pt-BR')}
                              </span>
                              {activity.is_suggestion ? (
                                <Button
                                  variant="ghost"
                                  ...
                                  onClick={() => acceptSuggestion(dayIdx, realIdx)}
                                >
                                  Aceitar
                                </Button>
                              ) : null}
                              <Button
                                ...
                                onClick={() => removeActivity(dayIdx, realIdx)}
                              >
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="mt-1 text-sm text-muted-foreground text-center max-w-md">
              Clique em "Gerar Roteiro com IA" para receber sugestões de atividades dia a dia,
              ou adicione dias manualmente.
            </p>
          </CardContent>
        </Card>
      )}
