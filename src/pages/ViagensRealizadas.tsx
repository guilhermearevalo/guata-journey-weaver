...
import { Search, MapPin, Calendar, Quote, Camera, Play, Images, ArrowRight } from 'lucide-react';
...
import { TripMediaCarousel } from '@/components/trips/TripMediaCarousel';
import {
  getCoverImageUrl,
...
const ViagensRealizadas = () => {
  ...
  const { data: realizadas, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data as CompletedTrip[];
    },
  });

  const filtered = realizadas?.filter(
    (t) =>
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="container mx-auto px-4 py-8">
        {isLoading ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <Card key={i} className="animate-pulse">
                ...
                <CardHeader><div className="h-6 w-3/4 rounded bg-muted" /></CardHeader>
              </Card>
            ))}
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filtered.map((trip) => {
              const media = normalizeTripMedia(trip);
              ...
              return (
                <Card key={trip.id} className="overflow-hidden transition-all hover:-translate-y-1 hover:shadow-xl">
                  ...
                  <CardContent className="space-y-3 pt-4">
                    {trip.client_quote && (
                      <div className="rounded-lg bg-muted/50 p-3">
                        ...
                        {trip.client_name && <p className="mt-2 text-xs font-medium">— {trip.client_name}</p>}
                      </div>
                    )}
                  ...
                  <CardFooter className="flex flex-col gap-2 sm:flex-row sm:items-stretch">
                    <Button variant="default" className="order-1 w-full gap-2 sm:order-2 sm:flex-1" asChild>
                    ...
                    {hasMedia && (
                      <Button
                        variant="outline"
                        className="order-2 w-full gap-2 border-primary/40 bg-primary/5 text-primary hover:bg-primary/10 sm:order-1 sm:flex-1"
          ...
          <div className="py-16 text-center">
            ...
            <Button className="mt-4" asChild>
              <Link to="/viagem-personalizada">Solicitar Viagem Personalizada</Link>
            </Button>
          </div>
        )}
      </section>
            ...
            <>
              <DialogHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  ...
                  {selectedTrip.trip_year && (
                    <>
                      {' '}
                      · {selectedTrip.trip_month ? `${monthNames[selectedTrip.trip_month - 1]} ` : ''}
                      {selectedTrip.trip_year}
                    </>
                  )}
                </p>
              ...
              <div className="space-y-4">
                <TripMediaCarousel items={normalizeTripMedia(selectedTrip)} title={selectedTrip.title} />

                {selectedTrip.description && (
                  <p className="whitespace-pre-line text-sm text-muted-foreground">{selectedTrip.description}</p>
                )}
                {selectedTrip.client_quote && (
                  <div className="rounded-lg bg-muted/50 p-4">
                    ...
                    {selectedTrip.client_name && <p className="mt-2 text-sm font-medium">— {selectedTrip.client_name}</p>}
                  </div>
                )}
